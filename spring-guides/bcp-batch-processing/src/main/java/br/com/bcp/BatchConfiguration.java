...
public class BatchConfiguration {
    ...
    /*
    Methods: reader() >>> processor() >>> writer()

    The first chunk of code defines the input, processor, and output. - reader() creates an ItemReader.
    It looks for a file called sample-data.csv and parses each line item with enough information to turn
    it into a Person. - processor() creates an instance of our PersonItemProcessor you defined earlier,
    meant to uppercase the data. - write(DataSource) creates an ItemWriter. This one is aimed at a JDBC
    destination and automatically gets a copy of the dataSource created by @EnableBatchProcessing.
    ...
    /*
    The first method (importUserJob()) defines the job and the second one (step1()) defines a single step.
    ...
    @Bean
    public PersonItemProcessorTwo processor2() {
	return new PersonItemProcessorTwo();
    }
    ...
    public Step step1() {
	/*
	chunk() is prefixed <Person,Person> because it’s a generic method. This represents the input and output
        types of each "chunk" of processing, and lines up with ItemReader<Person> and ItemWriter<Person>.
         */
